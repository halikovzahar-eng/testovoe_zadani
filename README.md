...
4. Выполнить команду она запустит web-приложение и nginx ``` docker compose up -d ``` 
5. Для проверки либо в Docker Desktop  во вкладке контейнеры посмотреть запущенные, там будет docker-compose и web-backend, revers_proxy-nginx. либо командами ``` docker ps ```
...
Когда идет обращение к *http://lockalhost* запрос идет на порт 80, nginx перенапраляет его на порт 8080 приложения и ввыодит ответ **"Hello from Effective Mobile!"**
